...
def api_recepcion_tiradas():
    ...
    try:
        ...
        # Si fallaron los separadores raros, intentamos el default
        ...
        if not c_monto or not c_nom: 
            return jsonify({"status": "error", "msg": f"Columnas no encontradas. Se detectaron: {list(df.columns)}"}), 400

        # 3. Preparación para guardar
        ...
        for _, row in df.iterrows():
            try:
                ...
                # B. ID Transacción (Evitar duplicados)
                ...
                # C. Nombre Vendedor (Corrección automática)
                ...
                # D. Monto (Limpiar símbolos $ y ,)
                val_str = str(row[c_monto]).replace('$','').replace('.','').replace(',','.') # Ejemplo: 1.000,00 -> 1000.00
                ...
                # F. Billetes (Si existen las columnas)
                def get_int(key):
                    c = get_col([key])
                    if c:
